...
#use the next code line to assign specific colors to host identities
#this could also be done by reading the hostID column and "uniquing" it
...
#use the next code line to assign specific colors to phylogenetic clades
#this could also be done by reading the cladeID column and "uniquing" it
names(cladecolors) <- scan(text = "N.USA SpainN SpainS S.USA", what = "")

# Read in tree data
  Tree1 <- read.tree("~/RAxML_bipartitions.LaJul9")

#use this next code line to specify an outgroup
